...
| `model.thinking` | `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max` | No | Not set | Sets the thinking level for this main agent. When the selected model does not expose the requested level, the extension applies the nearest available level: `minimal` and `low` are interchangeable, `medium` and `high` are interchangeable, then it searches higher levels, then lower levels, and finally uses `off`. |
