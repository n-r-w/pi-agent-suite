...
- **FRQ-34:** The main footer obtains API cost only from the usage store for the active root session family. Auxiliary requests publish complete usage without creating cost-only session entries. One root-only timer refreshes the cached total with an indexed `SUM(cost)` every 10 seconds while the footer is active; footer rendering reads only the cached value. When footer configuration enables `showApiCost`, the first unavailable usage read for a root session hides the API-cost segment and emits one warning in the interactive root session. This applies during startup and later refreshes. The refresh timer continues after a later failure, and a later valid result restores the segment. The warning is emitted even when the usage extension also reports its own startup error. No footer warning is emitted when the footer or `showApiCost` is disabled.
