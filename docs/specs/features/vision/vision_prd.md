...
A text-only primary model can call `describe_image`, pass an image path and a prompt, and receive a text answer from a configured vision model. A multimodal primary model needs nothing: `read` already delivers images natively.
...
- **Tool registration.** `describe_image` is registered with parameters `image_path` (required string) and `prompt` (required string), plus `promptSnippet` and `promptGuidelines`.
- **Image input forms.** The image is accepted as a file path only.
- **Delegation.** On execution, `describe_image` resolves the configured vision model, sends the image and prompt, and returns the model's text response.
...
- **Single image.** A `describe_image` call processes exactly one image.
- **Config storage.** Config is read from `~/.pi/agent/agent-suite/vision/config.json` with fields `enabled`, `provider`, `model`, `compression` (object `{ enabled, jpegQuality, maxBytes }`), `retry` (object `{ enabled, maxRetries, baseDelayMs }`).
