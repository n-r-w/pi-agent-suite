...
## Problem Statement
- PRB-01: Pi cancellation ends an active main-agent run without emitting `session_shutdown`. The `agent_end` event can contain final assistant `stopReason: "aborted"`, or `stopReason: "error"` while the active `ExtensionContext.signal` is aborted during tool cancellation.
...
## Proposed Solution
- SOL-01: A shared agent-end classifier reads the final assistant message, ignoring trailing tool-result messages. It classifies `stopReason: "aborted"` as cancellation and classifies neither provider errors nor missing assistant messages as cancellation.
- SOL-02: The root `run-subagent` runtime handles cancellation when the classifier reports an aborted outcome or the active `ExtensionContext.signal` is aborted. It admits `recoverRootShutdown` through `RuntimeFailureRecoveryTracker` and awaits the tracker drain.
...
- ACC-02: Given final `stopReason: "stop"`, `"toolUse"`, or `"error"` and no aborted active context signal, the lifecycle handler does not start root recovery.
...
- LIM-01: Pi does not expose cancellation of an auto-retry delay as a distinct extension lifecycle event. This solution handles active agent-run cancellation represented by final `stopReason: "aborted"` or an aborted active `ExtensionContext.signal`, plus session termination represented by `session_shutdown`.
