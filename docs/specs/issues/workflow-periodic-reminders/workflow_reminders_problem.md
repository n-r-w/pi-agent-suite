...
A long-running active stage can execute many tool calls or reasoning turns without publishing another workflow state message. The active workflow and stage information then moves farther from the end of model context.

## Who is affected

- Models working through tool-heavy or reasoning-heavy workflow stages.
...
3. Execute many tool calls or reasoning turns.
4. Observe that no newer workflow state record is added by model activity alone.
...
The model periodically receives a compact marker for the current workflow and active stage during long tool or reasoning loops without manual user reminders.

## Success Metrics

- A deterministic tool loop adds one current workflow reminder after the configured activity interval.
- A deterministic reasoning-only sequence queues one current workflow reminder for the next user-triggered model request after the configured activity interval.
...
- Active workflows with many tool calls or reasoning turns between lifecycle publications.
