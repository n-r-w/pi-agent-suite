...
| `footer` | Enabled | Shows project, optional git branch, quota, cost, selected agent, model, projection, MCP errors, context usage, and an optional extension status line. | `footer/config.json`: `enabled`, `showProvider`, `showModel`, `showThinkingLevel`, `showApiCost`, `showGitBranch`, `showExtensionStatusLine`. | [docs/extensions/footer.md](docs/extensions/footer.md) |
...
| `run-subagent` | Enabled | Runs named subagent tasks with child session logs, a navigable live widget, and `/subagents`. | `run-subagent/config.json`: `enabled`, `maxDepth`, `widgetLineBudget`. | [docs/extensions/run-subagent.md](docs/extensions/run-subagent.md) |
